...
const AdminProducts = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      alert(
        "البيانات الأساسية مطلوبة (name, section, price > 0)!"
      );
      return;
    }
    setIsUploading(true);
    try {
      ...
      if (selectedProduct && deletedImages.length > 0) {
        const relativeDeleted = deletedImages.map((img) =>
      ...
      setProducts((prev) =>
        selectedProduct
          ? prev.map((prod) =>
              prod._id === selectedProduct._id ? updatedProduct : prod
            )
          : [updatedProduct, ...prev]
      );
      resetForm();
  ...
  const handleRemoveOldImage = (imgPath) => {
    if (selectedProduct) {
      const updatedOldImages = selectedProduct.images.filter(
        (img) => img !== imgPath
      );
                  ...
                  <div className="images-section">
                    ...
                    <div className="images-actions">
                      ...
                      <button
                        ...
                        onClick={() =>
                          document
                            .getElementById("add-product-images")
                            .click()
                        }
                      >
